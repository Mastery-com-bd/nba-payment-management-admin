...
export const followUpApi = {
  ...
  async getAllFollowUps(params?: {
    ...
    return publicFetch(
      `${API_BASE_URL}/follow-up/${query ? `?${query}` : ""}`,
      {
        method: "GET",
        credentials: "include",
      },
  ...
  async createFollowUp(data: TFollowUp): Promise<any> {
    try {
      const res = await authFetch(`${API_BASE_URL}/follow-up`, {
        method: "POST",
  ...
  async getASingleFollowUps(id: string): Promise<any> {
    return publicFetch(`${API_BASE_URL}/follow-up/${id}`, {
      method: "GET",
      credentials: "include",
    });
  },

  async deleteFollowUp(id: string): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/follow-up/${id}`, {
      method: "DELETE",
      credentials: "include",
    });
